...
<html>
	...
	<script type="text/javascript">
		$(function(){
		    ...
		    $('.spinner').spinner({          
			    stop:function(e,ui){
			        ...
			        if(parseInt(sort)!= NaN && 0 < parseInt(sort)&& parseInt(sort) < 1000){
			        	$.get("${ctx}/menu/sort.do", { id: id, sort: sort, devType:'${devType}', deviceId:'${deviceId}' },
			        		function(data){
			        		if(data == 1){
			        			$("#status-" + id).text("未发布");
			        		}
		    ...
		    $("#menu-save").button().click(function(){
		    	window.location.href='/r2k/menu/savePage.do?deviceId=${deviceId}&devType=${devType}&deviceName=${deviceName}';
		    });
		  //清除发布
		    $("#clear-publish").button().click(function(){
		    	$("#dialog-clear-publish").dialog("open");
		    });
			//创建simpleTable
		    ...
		    $( "#dialogdelete" ).dialog({
		      ...
		      buttons: {
		        "确定": function() {
		        	...
		        	$.get("${ctx}/menu/delete.do", {id: id,devType:'${devType}',deviceId:'${deviceId}',deviceName:'${deviceName}'},
		        		function(data){
		        		location.reload();
		        	});
		        },
			...
			$( "#publish" ).button().click(function() {
	        	$.ajax({
	       			type:'post',
	       			url: "${ctx}/menu/publish.do?devType=${devType}&deviceId=${deviceId}&deviceName=${deviceName}",
	       			beforeSend: function () {
	       				ShowDiv();
	                },
	                complete: function () {
	       			...
	       			success:function(data){
	       				if(data == 1){
	       					$("#dialog-publish").html("").html("按设备发布菜单成功！");
	    	            	$("#dialog-publish").dialog("open");
	       				}else if(data == 0){
	       					$("#dialog-publish").html("").html("按设备发布菜单失败！");
	    	            	$("#dialog-publish").dialog("open");
	       				}else if(data == 2){
	       					$("#dialog-publish").html("").html("按设备发布菜单失败:没有设置主页！");
	    	            	$("#dialog-publish").dialog("open");
	       				}
	       			}
	       		  });
		    ...
		    $("#dialog-clear-publish").dialog({
		    	autoOpen: false,
				modal: true,
				buttons: {
					"确定": function() {
						window.location.href='/r2k/menu/clearPublish.do?deviceId=${deviceId}&devType=${devType}&deviceName=${deviceName}';
					}
				},
		     	close: function() {
		     		$( this).dialog( "close" );
	...
	<body>
	 <%@ include file="/commons/header.jsp" %>
	    ...
	    <div id="navLink">
	    	<a href="/r2k/dev/show.do" target="_self"><s:text name="r2k.menu.device"></s:text></a> &gt; ${deviceName}设备菜单管理
	    </div>			
		<div class="content">
				<div id="container" class="ui-widget">
					...
					<button id="clear-publish">清除发布</button>
					</div>

					<form id="form-list" action="/r2k/menu/device.do" method="post">
									...
									<tr>
										...
										<td>
											<s:if test="menuType == 'navigation'">
												<a href="/r2k/menu/updateNavPage.do?id=${id}&devType=${devType}&deviceId=${deviceId}&deviceName=${deviceName}">[修改]</a>
											</s:if>
											<s:else>
												<a href="/r2k/menu/updatePage.do?id=${id}&devType=${devType}&deviceId=${deviceId}&deviceName=${deviceName}">[修改]</a>
											</s:else>
		...
		<div id="dialog-clear-publish" title="清除发布">
			<p>
				确认清除发布？
			</p>
		</div>
