...
function addPicFileId(fileId,id,filename){
	var str = "#" + fileId;
	$(str).attr("path",id);
	$(str).append('<div class="upload_img" style="float:left;margin-right:10px;"><img width="150px" height="130px" src="' + id + '" fileInfo="'+filename+':'+id+'"/></div>');
	var info = $('<div id="upload_info" class="upload_info" style="height:100%;padding-top:10px;"></div>');
	$(info).append('<div style="margin-bottom:10px;"><label>名称:</label><input type="text" class="imgname" value="'+filename+'" style="width:380px;"/></div>');
	$(info).append('<div style="margin-bottom:10px;"><label>描述:</label><textarea class="imgdesc" value="" style="width:380px;"/></div>');
...
function addPicReadyFileInfo(fileid, fileName,message,status){
	var infoDiv = document.getElementById("infoDiv");
	$("#thumbnails").css("display","block");
	$("#infoDiv").append("<div id='" + fileid + "' class='add_pic_col' style='border:solid #000000 1px;height:130px;margin-bottom:10px;'></div>");
}
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
