...
<html>
	<head>
		<title>更新<%=${className}.TABLE_ALIAS%></title>
			...
			$(function() {	
				...
				//保存
			      ...
			      .click(function(e) {
			    	  ...
			    	  if(flag){
						$("#form-save").attr("action","${'${'}ctx${'}'}/${namespace}/update.do").submit();
					  }
			      });
			  	//清空
			  	$( "#cleanTest" )
	...
	<body>
		...
		<div id="navLink">
			<a href="${'${'}ctx${'}'}/${namespace}/pageQuery.do" target="_self"><s:text name="r2k.menu.${namespace}"></s:text></a> &gt; 更新<%=${className}.TABLE_ALIAS%>
		</div>
		<div class="content">
					...
					<form id="form-save" method="post">
					  	<div class="regbox">
					  		...
					  		<div class="field"><label class="field-label"><%=${className}.ALIAS_${column.constantName}%><span class="mstInput">*</span>:</label>
					       		<input class="ui-input field-input" name="${classNameLower}.${column.columnNameLower}" id="new_${column.columnNameLower}" onfocus="focusIn('new_${column.columnNameLower}')" value='<s:property value="${classNameLower}.${column.columnNameLower}"/>'/>
					        	<span class="ui-tips-error" id="new_${column.columnNameLower}_msg"></span>
						  	</div>
					  		</#list>
						  	<div class="field">
						  		<div class="form-button">
						  		<button id="${classNameLower}save">更新</button>
